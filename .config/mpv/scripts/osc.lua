...
function limited_list(prop, pos)
    ...
    local fs = math.ceil(tonumber(mp.get_property('options/osd-font-size')) * 0.6)
...
mp.register_script_message("osc-update-title", function(filename, title)
    ...
    for i, v in ipairs(limlist) do
        ...
        if not title then
            ...
            -- title not in cache
                ...
                mp.commandv('script-message', 'ytdl-resolve-title', v.filename)
            end
        end
...
function render_message(ass)
    ...
    then
        ...
        fontsize = math.ceil(fontsize * 0.6) -- = fontsize * counterscale / math.max(0.65 + math.min(lines/maxlines, 1), 1)
